...
async function callGeminiGenerate(params: {
  ...
  for (const model of candidateModels) {
    ...
    try {
      ...
      if (!rawText) throw new Error('AI 분석 결과 텍스트가 비어 있습니다.');
      return rawText;
    } catch (err: any) {
      lastError = err;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { imageFileBase64, mimeType = 'image/png', textContent, teacherContext = {} } = req.body || {};

  const todayStr: string = teacherContext.today || new Date().toISOString().split('T')[0];
  const subjectStr: string = teacherContext.subject || '2학년 역학과 에너지, 3학년 물리학Ⅱ';
  const dutyStr: string = teacherContext.duty || '과학과 교사, 담임 아님';
  const timetableStr: string = teacherContext.timetableText || '(시간표 정보 없음)';
...
# 4단계 — 선생님의 일인지 엄격히 판단하십시오

1. 담임 여부: 담임 학년/반 관련 업무는 isMine: true
2. 부서 및 직책: 소속 부서 업무는 isMine: true
3. 담당 과목: 담당 과목 출제·채점·생기부 세특은 isMine: true
4. 전 교직원 공통: 교직원 회의, 법정의무연수는 isMine: true

★ mineReason 에는 명확한 이유를 1줄로 작성하십시오.
...
1. 소요 기간을 추측해 적지 마십시오.
2. 자료에 없는 날짜를 만들어내지 마십시오.
3. 학생·학부모 이름, 전화번호는 그대로 옮기지 마십시오.
4. 확실하지 않은 것을 확실한 것처럼 쓰지 마십시오.
...
만약 시간표(컴시간/나이스)라면 kind: "시간표" 로 하고 timetable 에 요일별(mon, tue, wed, thu, fri) 슬롯 배열을 채우십시오.`;
  ...
  try {
    const rawText = await callGeminiGenerate({ apiKey, parts, systemInstruction });
    const cleaned = rawText.replace(/```json/g, '').replace(/```/g, '').trim();
    const parsed: ParseResultData = JSON.parse(cleaned);
    return res.json(parsed);
  } catch (err: any) {
    console.error('[api/gemini] 오류:', err);
    return res.status(500).json({ error: err.message || 'Gemini API 호출 실패' });
  }
}
